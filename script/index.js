...
element.forEach(element => element.addEventListener('click', () => {   
    (element.matches('#plusminus') && display.value === '0') ? (display.value = '-') :
    (element.value === '±') ? (display.value += '-') :
    (element.matches('#del') && (display.value === '0' || display.value.length === 1)) ? (display.value = '0'):
    (element.value === 'Del') ? (display.value = display.value.substring(0, display.value.length - 1), display.classList.remove('font-style')) :
    (!element.matches('.btn--operator') && display.value === '0') ? (display.value = element.value) :
    (element.value === '×') ? (display.value += '*') :
